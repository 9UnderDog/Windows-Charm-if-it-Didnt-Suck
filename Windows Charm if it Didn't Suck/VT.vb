...
Public Class VirusTotalScanner
    Private Const MaximumFileSize = 256 * (2 ^ 20) 'was 32 MB
    ...
    Public Sub New(APIKey As String)
        ...
        pClient.BaseUrl = New Uri("http://www.virustotal.com/vtapi/v2/")
    ...
    Public Property UseTLS() As Boolean
        ...
        Set(value As Boolean)
            mUseTLS = value
            If mUseTLS Then
                pClient.BaseUrl = New Uri("https://www.virustotal.com/vtapi/v2/")
            Else
                pClient.BaseUrl = New Uri("http://www.virustotal.com/vtapi/v2/")
            End If
        End Set
    ...
    Public Function SubmitFile(Filename As String) As List(Of ScanResult)
        ...
        Else
            Throw New SizeLimitException("Filesize limit of 256MB reached (file is " & File.Length \ 1024 \ 1024 & " MB).")
        End If
    ...
    Private Function GetReport(Request As RestRequest, Optional ApplyHack As Boolean = False) As Report
        ...
        If Response.StatusCode = HttpStatusCode.NoContent Then Throw New RateLimitException("5 requests/min limit reached.")
        If Response.StatusCode = HttpStatusCode.Forbidden Then Throw New AccessDeniedException("No access to service. Possibly bad API.")
        If Response.ErrorException IsNot Nothing Then Throw Response.ErrorException
        If Response.StatusCode <> HttpStatusCode.OK Then Throw New Exception("API error " & Response.StatusCode)
        If String.IsNullOrEmpty(Response.Content) Then Throw New Exception("No content in response.")
        If ApplyHack Then
